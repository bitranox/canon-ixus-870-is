               ...
               int param1, int param2, int param3, int param4, __attribute__ ((unused))int param5)
{
  ...
  switch ( param1 )
  {
    ...
    case PTP_CHDK_RemoteCaptureGetData:
        ...
        break;
    case PTP_CHDK_GetMJPEGFrame:
        {
            // Webcam MJPEG frame streaming
            // param2 = JPEG quality (1-100, 0 = use current)
            // param3 = flags (start/stop)
            int wc_flags = param3;

            if (wc_flags & PTP_CHDK_WEBCAM_STOP) {
                if (libwebcam) {
                    libwebcam->stop();
                }
                ptp.num_param = 1;
            ...
            if (wc_flags & PTP_CHDK_WEBCAM_START) {
                ...
                // Module auto-load: libwebcam is loaded on first access
                extern char module_last_error[100];
                module_last_error[0] = 0;
                if (libwebcam) {
                    int start_rc = libwebcam->start(quality);
                    if (start_rc != 0) {
                        // Return error string as data phase for diagnostics
                        ptp.num_param = 4;
                        ptp.param1 = 0;
                        ptp.param2 = (unsigned)start_rc;  // start return code
                        ptp.param3 = 0xDEAD;              // marker: start failed
                        ptp.param4 = 0;
                        if (module_last_error[0]) {
                            send_ptp_data(data, module_last_error, strlen(module_last_error) + 1);
                        } else {
                            send_ptp_data(data, "\0", 1);
                        }
                        break;
            ...
            // Get a frame
            if (libwebcam) {
                webcam_frame_t frame;
                webcam_status_t wc_status;
                ...
                if (gf_rc == 0 && frame.data && frame.size > 0) {
                    ...
                    // Return diagnostic info: active state, get_frame rc, frame count
                    ptp.num_param = 4;
                    ptp.param1 = 0;  // no frame
                    ptp.param2 = wc_status.active;
                    ptp.param3 = (unsigned)gf_rc;
                    ptp.param4 = wc_status.frames_sent;
                    send_ptp_data(data, "\0", 1);
                }
            } else {
                ptp.code = PTP_RC_GeneralError;
