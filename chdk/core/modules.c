...
#define MODULE_NAME_WEBCAM "webcam.flt"

// Forward reference
extern libwebcam_sym default_libwebcam;

module_handler_t h_webcam =
{
    (base_interface_t**)&libwebcam,
    &default_libwebcam.base,
    WEBCAM_VERSION,
    MODULE_NAME_WEBCAM
};

// Default (unloaded) functions - auto-load module on first call
static int default_webcam_start(int jpeg_quality)
{
    if (module_load(&h_webcam))
        return libwebcam->start(jpeg_quality);
    return -1;
}

static int default_webcam_stop(void)
{
    if (module_load(&h_webcam))
        return libwebcam->stop();
    return -1;
}

static int default_webcam_get_frame(webcam_frame_t *frame)
{
    if (module_load(&h_webcam))
        return libwebcam->get_frame(frame);
    return -1;
}

static void default_webcam_get_status(webcam_status_t *status)
{
    if (status) {
        memset(status, 0, sizeof(webcam_status_t));
    }
}

// Default library - module unloaded
libwebcam_sym default_libwebcam =
{
    { 0, 0, 0, 0, 0 },
    default_webcam_start,
    default_webcam_stop,
    default_webcam_get_frame,
    default_webcam_get_status
};

// Library pointer
libwebcam_sym* libwebcam = &default_libwebcam;
